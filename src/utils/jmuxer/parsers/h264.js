...
let log2_max_frame_num_minus4 = 0
export class H264Parser {
  ...
  static readSPS (data) {
    ...
    const rbsp = []
    const hdr_bytes = 1
    const nal_bytes = data.byteLength
    ...
    log2_max_frame_num_minus4 = decoder.readUEG() // log2_max_frame_num_minus4
  ...
  static parseHeader (unit) {
    const decoder = new ExpGolomb(unit.getPayload())
    // skip NALu type
    decoder.readUByte()
    unit.isfmb = decoder.readUEG() === 0
    unit.stype = decoder.readUEG()
    decoder.readUEG() // ppsi
    unit.fn = decoder.readBits(log2_max_frame_num_minus4 + 4)
  }
